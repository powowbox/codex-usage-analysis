...
`analyze_codex_usage.py` also accepts `--timezone` (Europe/Paris by default). `compare_models.py` accepts `--pricing` and excludes Sundays in
Europe/Paris, while keeping Saturdays. It reports averages per active day and
per calendar day excluding Sundays, including days with no events.
...
compare_models.py           Multi-model simulation, Sundays excluded
...
* `comparison_models.csv`, `comparison_daily.csv`, `comparison_summary.json`:
  comparative estimates and averages excluding Sundays.
